...
import {STEP_ENROLLMENT_AMOUNT} from "@/services/transaction/step_transition_service.js";
...
const amountModel = computed({
  get: () => createStore.enrollmentAmount,
  set: val => createStore.enrollmentAmount = val
})

onMounted(() => {
  createStore.enrollmentAmount = createStore.amount
  createStore.enrollmentCurrencyId = createStore.currencyId
})
...
<template>
  ...
  <v-expansion-panel :value="STEP_ENROLLMENT_AMOUNT">
        ...
        <v-row no-gutters>
          ...
          <v-col class="text--secondary text-right" cols="8">
            <v-fade-transition leave-absolute>
              ...
              <span v-else key="1">{{createStore.enrollmentAmount}} {{ createStore.getEnrollmentCurrency.symbol }}</span>
            </v-fade-transition>
          </v-col>
        </v-row>
      </template>
    ...
    <v-expansion-panel-text>
      <div class="d-flex justify-end align-center pa-2">
        <div class="text-right d-flex align-center" @click="isCalcShow = true">
          <span class="text-h5 ">{{createStore.enrollmentAmount}}</span>
        ...
        <v-menu>
          <template v-slot:activator="{ props }">
            ...
            >
              <v-icon :icon="createStore.getEnrollmentCurrency.icon"/>
            </v-btn>
          </template>
          <CurrenciesList
              :currencies="currencyStore.currencies"
              @selected="createStore.enrollmentCurrencyId = $event.id"
          />
        </v-menu>
      </div>
      <BottomCalculator
          v-model="isCalcShow"
          :start-sum="createStore.enrollmentAmount"
          @done="createStore.enrollmentAmount = $event"
      />
    </v-expansion-panel-text>
